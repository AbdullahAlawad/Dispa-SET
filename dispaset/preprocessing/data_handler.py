...
def NodeBasedTable(path,idx,zones,tablename='',default=None):
    ...
    zone (a.k.a node, zone) of the simulation.
    ...
    if os.path.isfile(path):
        ...
        for c in zones:
            ...
            if os.path.isfile(path_c):
                ...
                logging.critical('No data file found for the table ' + tablename + ' and zone ' + c + '. File ' + path_c + ' does not exist')
#                sys.exit(1)
        SingleFile=False
    data = pd.DataFrame(index=idx)
    if len(paths) == 0:
...
def load_config_excel(ConfigFile,AbsPath=True):
    ...
    # List of parameters for which an external file path must be specified (Order is important for this list):
    params = ['Demand', 'Outages', 'PowerPlantData', 'RenewablesAF','LoadShedding', 'NTC', 'Interconnections',
              'ReservoirScaledInflows', 'PriceOfNuclear','PriceOfBlackCoal', 'PriceOfGas', 'PriceOfCrudeOil', 'PriceOfBiomass',
              'PriceOfCO2', 'Reservoir Levels', 'PriceOfLignite', 'PriceOfPeat', 'HeatDemand', 'CostHeatSlack',
              'CostLoadShedding', 'PriceOfDiesel', 'PriceOfHFO', 'PriceOfMunicipalSolidWaste', 'PriceOfLandFillGas',
               'FuelsPricesPerZone']
    for i, param in enumerate(params):
        config[param] = sheet.cell_value(61 + i, 2)

    #Alternative NTC (1) & (2)
    config['NTC1'] = sheet.cell_value(66, 3)
    config['NTC2'] = sheet.cell_value(66, 4)
    ...
    params = ['PriceOfNuclear','PriceOfBlackCoal', 'PriceOfGas', 'PriceOfCrudeOil', 'PriceOfBiomass', 'PriceOfCO2']
    for i, param in enumerate(params):
        config[param+' 2'] = sheet.cell_value(69 + i, 3)
    params = ['PriceOfLignite', 'PriceOfPeat']
    for i, param in enumerate(params):
        config[param+' 2'] = sheet.cell_value(76 + i, 3)
    params = ['CostHeatSlack', 'CostLoadShedding', 'PriceOfDiesel', 'PriceOfHFO',
              'PriceOfMunicipalSolidWaste', 'PriceOfLandFillGas']
    for i, param in enumerate(params):
        config[param+' 2'] = sheet.cell_value(81 + i, 3)
    ...
    config['default']['PriceOfCrudeOil'] = sheet.cell_value(72, 5)
    ...
    config['default']['PriceOfDiesel'] = sheet.cell_value(81, 5)
    config['default']['PriceOfHFO'] = sheet.cell_value(82, 5)
    config['default']['PriceOfMunicipalSolidWaste'] = sheet.cell_value(83, 5)
    config['default']['PriceOfLandFillGas'] = sheet.cell_value(84, 5)
    config['default']['PriceOfNuclear 2'] = sheet.cell_value(69, 6)
    config['default']['PriceOfBlackCoal 2'] = sheet.cell_value(70, 6)
    config['default']['PriceOfGas 2'] = sheet.cell_value(71, 6)
    config['default']['PriceOfCrudeOil 2'] = sheet.cell_value(72, 6)
    config['default']['PriceOfBiomass 2'] = sheet.cell_value(73, 6)
    config['default']['PriceOfCO2 2'] = sheet.cell_value(74, 6)
    config['default']['PriceOfLignite 2'] = sheet.cell_value(76, 6)
    config['default']['PriceOfPeat 2'] = sheet.cell_value(77, 6)
    config['default']['CostHeatSlack 2'] = sheet.cell_value(79, 6)
    config['default']['CostLoadShedding 2'] = sheet.cell_value(80, 6)
    config['default']['PriceOfDiesel 2'] = sheet.cell_value(81, 6)
    config['default']['PriceOfHFO 2'] = sheet.cell_value(82, 6)
    config['default']['PriceOfMunicipalSolidWaste 2'] = sheet.cell_value(83, 6)
    config['default']['PriceOfLandFillGas 2'] = sheet.cell_value(84, 6)
    config['default']['FuelsPricesPerZone'] = sheet.cell_value(85, 5)
    config['GAMS_ModelCode'] = sheet.cell_value(103, 2)
    ...
    config['zones'] = read_truefalse(sheet, 87, 1, 102, 3)
    config['zones'] = config['zones'] + read_truefalse(sheet, 87, 4, 102, 6)

    try:
        countries_list = [(i, item.value.encode('utf-8').decode('UTF-8')) for i, item in
                          enumerate(sheet.row_slice(rowx=155, start_colx=0, end_colx=100))]
        countries_list = [x for x in countries_list if x[1] != '']
        country_zones = {}
        for item in countries_list:
            zones_list = [element.value.encode("utf-8").decode('UTF-8') for element in
        ...
        config['countries'] = list(country_zones.keys())
    except:
        pass
...
def load_config_yaml(filename,AbsPath=True):
    ...
    params = ['Demand', 'Outages', 'PowerPlantData', 'RenewablesAF', 'LoadShedding', 'NTC', 'Interconnections',
              ...
              'CostHeatSlack','CostLoadShedding', 'PriceOfDiesel', 'PriceOfHFO', 'PriceOfMunicipalSolidWaste',
              'PriceOfLandFillGas', 'FuelsPricesPerZone']
